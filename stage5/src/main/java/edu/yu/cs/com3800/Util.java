...
public class Util {
    ...
    public static byte[] readAllBytesFromNetworkWithFailure(InputStream in, long timeout, Logger logger) {
        long start = System.currentTimeMillis();
        try {
            while (in.available() == 0 && System.currentTimeMillis() - start < timeout) {
                try {
                    Thread.currentThread().sleep(500);
                }
                catch (InterruptedException e) {
    ...
    public static String getLoggerName(String className, long serverID, int port) {
        return className + "-on-serverID-" + serverID + "-on-TCPPort-" + port + ".log";
    }
}
