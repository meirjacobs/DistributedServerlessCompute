...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, LinkedBlockingQueue<Message> incomingHeartbeats,
                              InetSocketAddress myAddress, int myPort, ZooKeeperPeerServer peerServer) throws IOException {
        this.incomingMessages = incomingMessages;
        this.incomingHeartbeats = incomingHeartbeats;
        this.myAddress = myAddress;
        this.myPort = myPort;
        this.logger = initializeLogging(Util.getLoggerName(UDPMessageReceiver.class.getSimpleName(), peerServer.getServerId(), peerServer.getUdpPort() + 2));
        this.setDaemon(true);
        ...
        while (!this.isInterrupted()) {
            try {
                ...
                if (sendLeader(received)) {
                    ...
                    boolean done = false;
                    while(!done){
                        done = this.incomingHeartbeats.offer(received);
                    }
                }
    ...
    private boolean sendLeader(Message received) {
        ...
        logger.info("Received ELECTION message from server ID " + receivedNotification.getSenderID());
    ...
    private boolean strayElectionMessage(Message received) {
        ...
        if (receivedNotification.getState() != ZooKeeperPeerServer.ServerState.LOOKING
                && /*this.peerServer.getCurrentLeader() != null*/
