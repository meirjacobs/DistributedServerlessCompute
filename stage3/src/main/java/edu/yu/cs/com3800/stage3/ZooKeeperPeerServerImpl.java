...
public class ZooKeeperPeerServerImpl extends Thread implements ZooKeeperPeerServer {
    ...
    private JavaRunnerFollower runner;
    private LinkedBlockingQueue<Message> toRunner;
    private LinkedBlockingQueue<Message> runnerResponses;
            ...
            try {
                switch(getPeerState()) {
                    case LOOKING:
                        ...
                        break;
                    case FOLLOWING:
                        runner = new JavaRunnerFollower(this, toRunner, runnerResponses);
                        Util.startAsDaemon(runner, myAddress.toString() + ": javarunner");
                        sendWorkToRunner();
                        break;
                    case LEADING:
                        roundRobinLeader = new RoundRobinLeader(this, incomingMessages, outgoingMessages, new ArrayList<>(peerIDtoAddress.keySet()));
    ...
    private void sendWorkToRunner() {
        Message m, response;
        while(getPeerState() == ServerState.FOLLOWING && !shutdown) {
            // If there is work to send to the JavaRunnerFollower
            if((m = incomingMessages.poll()) != null && m.getMessageType() == Message.MessageType.WORK) {
                toRunner.offer(m);
            }
            // If there is completed work, report to the leader/master thread
            if((response = runnerResponses.poll()) != null) {
                // In lieu of sendMessage(), because the Message is already built
